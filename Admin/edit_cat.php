...
<?php 
  ...
  if(isset($id))
  {
    $sql=$link->rawQueryone("select * from category_tb where category_id_pk=?",Array('category_id_pk'=>$id));
  }
?>
<?php 
  if(isset($_POST['update_cat']))
  {
      ...
      $sql1=$link->update("category_tb",Array("category_name"=>$catname));
	  if(isset($catimage))
	  {
		
		
        $ext=pathinfo($catimage,PATHINFO_EXTENSION);
        $path="images/category/".$id.".".$ext;
        if(move_uploaded_file($_FILES['catimage']['tmp_name'],$path))
        {
          $link->where("category_id_pk",$id);
          $link->update("category_tb",Array("category_image"=>$path));
        }
...
<head>
    ...
    <title>360 Overseas Admin - Update Category</title>
  ...
  <link rel="stylesheet" href="css/bootstrap-datepicker.min.css">
	...
	function check()
	{
		...
		{
			document.getElementById("scatname").innerHTML="Alphabets Only";
			s=false;
		}
		else
		{
		...
		var ext=catimage.substring(catimage.lastIndexOf('.')+1);
		
		if(catimage!="")
		{
			if(ext!="png" && ext!="jpg" && ext!="jpeg" && ext!="PNG" && ext!="JPEG" && ext!="JPG")
			{
				document.getElementById("scatimage").innerHTML="jpg or png Files Only!";
				s=false;
			}
		}
		else
		{
			document.getElementById("scatimage").innerHTML="";
		}
		return s;
          ...
          <div class="row">
            <div class="col-12">
              <div class="form-group row">
                ...
                <input class="form-control" type="text" id="catname" name="catname"value="<?php echo $sql['category_name']; ?>">
				<span id="scatname" style="color:red;"></span>
                </div>
              </div>
             <div>
              <div class="form-group row">
                ...
                <div class="col-sm-10">
                <input class="form-control" type="file" name="catimage" id="catimage"><br>
                <img src="<?php echo $sql['category_image']; ?>" height=100 width=100 alt="image"/>
				<span id="scatimage" style="color:red;"></span>
                </div>
              </div>

                <center>
                <input type="submit" class="btn btn-info pull-right" value="Update" name="update_cat" style="align:center">
               </center>
              </div>
